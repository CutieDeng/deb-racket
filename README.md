...
revision is generated as `deb-release.deb-system`, so `4`
and `ubuntu2404` produce version `9.2.2-4.ubuntu2404`.

Validate an existing `.deb`:

```sh
scripts/verify-deb.sh \
  --deb /path/to/artifacts/racket9_9.2.2-4.ubuntu2404_amd64.deb \
  --deb-system ubuntu2404 \
  --deb-release 4 \
  --deb-arch amd64 \
  --cache-mode postinstall
