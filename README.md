...
```sh
scripts/build-deb.sh \
  --source-archive /path/to/racket-minimal-9.3.1-src.tgz \
  --artifact-dir /path/to/artifacts \
  --work-dir /path/to/work \
  --deb-system ubuntu2404 \
  --deb-release 2 \
...
revision is generated as `deb-release.deb-system.cache-rank.cache-mode`, so `2`
and `ubuntu2404` produce version `9.3.1-2.ubuntu2404.2.cached`
for the default cached mode and `9.3.1-2.ubuntu2404.1.postinstall`
...
scripts/verify-deb.sh \
  --deb /path/to/artifacts/racket9_9.3.1-2.ubuntu2404.2.cached_amd64.deb \
  --deb-system ubuntu2404 \
  --deb-release 2 \
  --deb-arch amd64 \
  --cache-mode cached
